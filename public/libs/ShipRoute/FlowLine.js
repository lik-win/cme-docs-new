...
const ol_style_Style = window.CME2D.style.Style
const { asArray: ol_color_asArray, asString: ol_color_asString } =
  window.CME2D.color
import { ol_coordinate_dist2d } from './Utils.js'
import './LineStringSplitAt.js'
...
var ol_style_FlowLine = class olstyleFlowLine extends ol_style_Style {
  ...
  _render(geom, e) {
    if (e.geometry.getType() === 'LineString') {
      ...
      if (
        geom.length > 1 &&
        (this.getArrow() === -1 || this.getArrow() === 2)
      ) {
