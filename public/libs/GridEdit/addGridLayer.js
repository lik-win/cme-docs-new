const { WebGLTile: TileLayer } = window.CME2D.layer
const { WMTS: WMTSTileGrid } = window.CME2D.tilegrid
...
export function addGridLayer(map, dimensions = {}) {
  let gridLayer = map
    .getAllLayers()
    .find((layer) => layer.get('layerName') === 'gridLayer')
  let gridTextLayer = map
    .getAllLayers()
    .find((layer) => layer.get('layerName') === 'gridTextLayer')
  if (gridLayer) {
    ...
    loader: function (z, x, y) {
      ...
      return fetch(url1)
        .then((res) => {
          ...
          scale = contentDisposition ? extractScale(contentDisposition) : 10
        ...
        .then(async (data) => {
          ...
          if (isUV) {
            ...
            const DirArray = float32Array.slice(
...
function getColorRGBA(value, colorStops) {
  ...
  for (let i = 0; i < colorStops.length - 1; i++) {
    if (value >= colorStops[i][0] && value < colorStops[i + 1][0]) {
      // 计算比例
      const ratio =
        (value - colorStops[i][0]) / (colorStops[i + 1][0] - colorStops[i][0])
      // 线性插值计算颜色
      ...
      const alpha =
        Math.round(
          colorStops[i][1][3] * (1 - ratio) + colorStops[i + 1][1][3] * ratio,
        ) * 255
