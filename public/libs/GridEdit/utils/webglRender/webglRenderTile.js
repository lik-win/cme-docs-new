...
export default class WebglRenderTile {
  ...
  initGL() {
    ...
    this.textCancvasContent = this.textCancvas.getContext('2d', {
  ...
  _render(config) {
    return new Promise(async (resolve, reject) => {
      ...
      for (let r = 0; r < size; r++) {
        for (let c = 0; c < size; c++) {
          ...
          let realU =
            u === noData
              ? u
              : this.parseRealDataCallBack({ offset, scale, value: u })
      ...
      const textTextureData =
        showType === 'value'
          ? this._renderToCanvasText(config)
          : await this._renderToCanvasWindBarb(config)
  ...
  _renderToCanvasText(config) {
    const { imageData, size, offset, scale, showZero, legend, noData } = config
    // 输出转换后的 float32 值
    ...
    for (let i = 0; i < size; i += pixelDistance) {
      for (let j = 0; j < size; j += pixelDistance) {
        ...
        const dataValue =
          imageData[dataIndex] !== 0 && imageData[dataIndex] !== noData
            ? Number(imageData[dataIndex] / scale + offset).toFixed(1)
            : ''
        let data = Number(dataValue) === 0 && showZero ? '' : Number(dataValue)
        if (data || data === 0) {
          // 在画布上的中心位置绘制文本
          this.textCancvasContent.strokeText(
            `${data}`,
            j + halfDistance,
            i + halfDistance,
          )
          this.textCancvasContent.fillText(
            `${data}`,
            j + halfDistance,
            i + halfDistance,
          )
  ...
  makeColorRamp(legend, type = 'step') {
    let colors = legend.map((item) => {
      return [
        item[0],
        `rgba(${item[1][0]},${item[1][1]},${item[1][2]},${item[1][3]})`,
      ]
    })
  ...
  createTexture(gl, filter, data, width, height, flag) {
    ...
    flag
      ...
      : gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 0) //纹理图片上下反转
