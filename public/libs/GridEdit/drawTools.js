const { Collection, Feature } = window.CME2D
const { DragPan, Draw, Modify, Select, Snap } = window.CME2D.interaction
const { GeoJSON } = window.CME2D.format
const { Circle, Fill, Stroke, Style, Text } = window.CME2D.style
const { LineString, Point, Polygon } = window.CME2D.geom
const { Vector: VectorLayer } = window.CME2D.layer
const { Vector: VectorSource } = window.CME2D.source
const { createBox, createRegularPolygon } = window.CME2D.interaction.Draw
const CircleGeo = window.CME2D.geom.Circle

const { Icon } = window.CME2D.style
...
class DrawingTool {
  ...
  drawStart(callBack) {
    ...
    this.draw.on('drawstart', (event) => {
      ...
      // 监听地图视图上的右键点击事件
      that.map.getViewport().addEventListener('contextmenu', rightClickListener)
      // 移除所有的平移交互
      callBack(event)
    })
  }
  ...
  drawInteraction(val, bool) {
    ...
    switch (val) {
      ...
      case 'LineString': // 绘制圆形
        this.createDrawInteraction('LineString', undefined, false, 'LineString')
        break
      case 'Rectangle': // 绘制矩形
        // 定义一个矩形几何函数
  ...
  setFeature(data) {
    ...
    if (data.geometryName === 'Point') {
      ...
      const feature = new Feature({
        geometry: new Point.PointGeo([Number(cords[0]), Number(cords[1])]),
